...
This guide explains how to set up the GitHub Actions workflows for automated deployment of the insightcore Digital Innovations application.
...
```bash
# Staging
docker logs -f insightcore-staging

# Production
docker logs -f insightcore-production
```
...
- Docker image tag: `slemfowel1/insightcore-digital-innovations:staging`
- Container name: `insightcore-staging`
- Port: `4019`
...
- Container name: `insightcore-production`
- Port: `4019`
...
```bash
docker logs insightcore-staging
```
...
docker exec insightcore-production npm run migrate
```
...
- name: Run Database Migrations
  run: docker exec insightcore-production npm run db:migrate
```
...
```bash
# On server
docker stop insightcore-production
docker rm insightcore-production
docker pull slemfowel1/insightcore-digital-innovations:previous_tag
docker run -d -p 4019:4019 --name insightcore-production \
  --env-file /home/apps/.env.production \
  slemfowel1/insightcore-digital-innovations:previous_tag
```
