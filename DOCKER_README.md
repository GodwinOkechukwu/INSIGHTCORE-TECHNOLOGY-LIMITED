...
This guide explains how to build and run the insightcore Digital Innovations application using Docker.
...
```bash
docker build -t insightcore:latest .
```

Or with a specific version tag:

```bash
docker build -t insightcore:1.0.0 .
```
...
```bash
docker run -d \
  --name insightcore_app \
  -p 4019:4019 \
  --env-file .env.local \
  insightcore:latest
```

Replace `insightcore:latest` with your image name/tag.
...
| `TABLE_PREFIX`         | Database table prefix                | No       | `insightcore_`            |
...
docker exec -it insightcore_postgres psql -U postgres -d store_db
...
2. Or with `docker run`:
   ```bash
   docker run -p 8000:4019 insightcore:latest
   ```
...
```bash
docker exec insightcore_postgres psql -U postgres -d store_db -c "\dt"
```
